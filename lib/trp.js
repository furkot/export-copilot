generate.contentType = 'application/octet-stream';
generate.extension = 'trp';
...
function isStop({ nights, visit_duration, url, pin }) {
  return nights || visit_duration || url || (pin != null && pin !== PASSTHROUGH);
}
...
function* writeStep(seq, step, show) {
  ...
  if (step.name || step.address || step.locality) {
    ...
    yield* writeStop(seq.index, {
      ...
      City: step.locality?.town,
      State: step.locality?.province,
      Country: step.locality?.country_long,
      ...data
    });
  } else {
    // waypoint
